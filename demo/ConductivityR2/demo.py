...
cfg = mconfig.Coincided(np.zeros((2,1)), 10, 10, np.array([1.0, 1/16 *np.pi, 2*np.pi]), False, np.array([1, -1]), 0.01)
cfg2 = mconfig.Coincided(np.zeros((2,1)), 10, 50)
Gn = Green2D_Dn(cfg.src(0), B.points, B.normal)

B = np.array([B])
#cfg.plot()
P = Conductivity.Conductivity(B, cnd, pmtt, cfg)
P2 = Conductivity.Conductivity(B, cnd, pmtt, cfg2)
freq = np.linspace(0.01,2*np.pi, 4)
#print(freq)
#print(R[0][:,].shape)
